...
#[instrument(skip_all)]
pub(crate) async fn build_clients(cfg: &CliArgs) -> eyre::Result<(SourceClient, TargetClient, Option<TargetClient>)> {
    ...
    info!("Eth Wallet decrytped with address {:x}", root_wallet.address());
    let root_nonce = NonceManagerMiddleware::new(root_provider, root_wallet.address());
    Some(TargetClient::new_with_provider_chain(root_nonce, root_wallet.with_chain_id(cfg.target_chain_id)).await?)
    } else {None};
