...
pub mod delegation_manager_storage {
    ...
    impl ::ethers::contract::EthLogDecode for DelegationManagerStorageEvents {
        fn decode_log(
            ...
            if let Ok(decoded) = AvsmetadataURIUpdatedFilter::decode_log(log) {
                return Ok(
                    DelegationManagerStorageEvents::AvsmetadataURIUpdatedFilter(decoded),
                );
            }
            if let Ok(decoded) = OperatorAVSRegistrationStatusUpdatedFilter::decode_log(
                ...
                return Ok(
                    DelegationManagerStorageEvents::OperatorAVSRegistrationStatusUpdatedFilter(
                        decoded,
                    ),
                );
            }
            ...
            if let Ok(decoded) = WithdrawalDelayBlocksSetFilter::decode_log(log) {
                return Ok(
                    DelegationManagerStorageEvents::WithdrawalDelayBlocksSetFilter(
                        decoded,
                    ),
                );
            }
    ...
    impl ::core::convert::From<AvsOperatorStatusCall> for DelegationManagerStorageCalls {
