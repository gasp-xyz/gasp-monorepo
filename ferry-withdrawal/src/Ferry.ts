...
class Ferry {
	...
	logFilteredOut(before: Withdrawal[], after: Withdrawal[], message: string) {
		...
		const mapAfter: Map<bigint, Withdrawal> = new Map(
	...
	async getPendingWithdrawals(): Promise<Withdrawal[]> {
		...
		const validWithdrawal = withdrawals.filter(
			(elem) =>
				elem.requestId >= start &&
				elem.requestId <= end &&
				elem.amount >= elem.ferryTip,
		);
	...
	async rateWithdrawals(withdrawals: Withdrawal[]): Promise<Withdrawal[]> {
		const balances = await Promise.all(
			this.tokensToFerry.map(([tokenAddress, _minProfit, _weight]) =>
				Promise.all([
					Promise.resolve(tokenAddress),
					this.l1.getBalance(tokenAddress, this.me),
				]),
			),
		);
		...
		const canAffordWirhdrawals = withdrawals.filter(
			(elem) =>
				balances.find(([tokenAddress, balance]) => {
					if (isEqual(tokenAddress, nativeTokenAddress)) {
						return (
							isEqual(tokenAddress, elem.tokenAddress) &&
							balance != null &&
							balance >= elem.amount + this.minBalance
						);
					} else {
						return (
							isEqual(tokenAddress, elem.tokenAddress) &&
		...
		this.logFilteredOut(
			withdrawals,
			canAffordWirhdrawals,
			"not enough balance",
		);

		const aboveMinimalProfit = canAffordWirhdrawals.filter((elem) => {
			const minProfit = this.tokensToFerry.find(
				([tokenAddress, minProfit, weight]) =>
					isEqual(tokenAddress, elem.tokenAddress),
			)![1];
		...
		this.logFilteredOut(
			canAffordWirhdrawals,
			aboveMinimalProfit,
			"below minimal profit",
		);

		const rankedWithdrawals = aboveMinimalProfit.sort((first, second) => {
			const firstTokenWeight = this.tokensToFerry.find(
				([tokenAddress, minProfit, weight]) =>
					isEqual(tokenAddress, first.tokenAddress),
			)![2];
			const secondTokenWeight = this.tokensToFerry.find(
				([tokenAddress, minProfit, weight]) =>
					isEqual(tokenAddress, second.tokenAddress),
			)![2];
	...
	async getPastFerriesReadyToClose(
		blockInPast: number,
		who: Uint8Array | null = null,
	): Promise<Withdrawal[]> {
		const account: Uint8Array = who === null ? this.me : who!;
		const rangeEnd = await this.l1.getLatestRequestId(LATEST);
		const latestRequestIdInPast =
			await this.l2.getLatestRequestIdInPast(blockInPast);
		...
		const withdrawalsWithStatus: [Withdrawal, Uint8Array | null][] =
			await Promise.all(
				withdrawals.map((elem) =>
					Promise.all([Promise.resolve(elem), this.l1.getFerry(elem.hash)]),
				),
			);

		logger.silly(
			`Found ${withdrawals.length} past ferried withdrawals, looked through rid (${rangeStart} ... ${rangeEnd})`,
		);

		const result = withdrawalsWithStatus
			.filter(
				([_withdrawal, ferry]) => ferry !== null && isEqual(ferry!, account),
			)
			.map(([withdrawal, _ferry]) => withdrawal);

		logger.silly(
			`Found ${result.length} past ferried withdrawals ferried by ${u8aToHex(
				account,
			)}`,
		);
