...
describe('test trades amounts', () => {
  ...
  beforeAll(async () => {
    ...
    vi.doMock('../src/repository/PriceRepository', () => ({
      ...
      getLatest: vi.fn().mockImplementation((id) => _.last(fixtures.tradesFirstDay)?.timestamp),
    }))
    vi.doMock('../src/repository/ChainRepository', () => ({
      ...
      getEvents: vi
        ...
        .mockImplementation((from, to) =>
          fixtures.tradesFirstDay.filter((ev) => ev.timestamp > from && ev.timestamp <= to).slice(0, fixtures.LIMIT)
        ),
  ...
  it('test amounts for first day', () => {
        ...
        e.events
          ...
          .map((e) => {
            const ids = _.isArray(e.data[1])
              ? [Number.parseInt(_.first(e.data[1])), Number.parseInt(_.last(e.data[1]))]
              : [Number.parseInt(e.data[1]), Number.parseInt(e.data[3])]
            const amounts = _.isArray(e.data[1])
              ? [new Decimal(parseNumber(e.data[2])), new Decimal(parseNumber(e.data[3]))]
              : [new Decimal(parseNumber(e.data[2])), new Decimal(parseNumber(e.data[4]))]
            return {
              pool: ids,
              amount: amounts,
            }
    ...
    amounts[0].div(new Decimal('1e12')).mul(KSM_PRICE).should.be.eql(new Decimal('28690.59604163819624'))
    amounts[1].div(new Decimal('1e18')).mul(MGX_PRICE).should.be.eql(new Decimal('29439.4205813848103'))
  })
  ...
  it('test trade processor amounts', async () => {
    ...
    const amounts = store
      .get(5)!
      .reduce((acc, trade) => [acc[0].add(trade[1][0]), acc[1].add(trade[1][1])], [ZERO, ZERO])

    amounts[0].div(new Decimal('1e12')).should.be.eql(new Decimal('28690.59604163819624'))
    amounts[1].div(new Decimal('1e18')).should.be.eql(new Decimal('29439.420581384810303'))
  })
})
