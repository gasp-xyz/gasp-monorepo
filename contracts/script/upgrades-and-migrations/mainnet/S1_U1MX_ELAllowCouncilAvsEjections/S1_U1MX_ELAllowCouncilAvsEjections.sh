...
PREFIX=wss://mainnet.gateway.tenderly.co/
...
forge script S1_U1MX_ELAllowCouncilAvsEjections.s.sol --sig "run()" --rpc-url $RPC_URL --account $WALLET_NAME --broadcast -vvvvvvv --verify --etherscan-api-key $ETHERSCAN_API_KEY

# Run verify after the proxy has been pointed to the new implementation via proxyAdmin by the council 
# forge script S1_U1MX_ELAllowCouncilAvsEjections.s.sol --sig "verify()" --rpc-url $RPC_URL --account $WALLET_NAME -vvvvvvv
