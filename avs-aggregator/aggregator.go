...
func (agg *Aggregator) createRdTask(chainToUpdate uint64, chainBatchIdToUpdate uint32) (taskmanager.IFinalizerTaskManagerRdTask, sdktypes.TaskId, error) {
	// Make sure that the taskManager is ready to accept another task
	isTaskPending, err := agg.ethRpc.AvsReader.IsTaskPending(context.Background())
	if err != nil {
...
func (agg *Aggregator) createAndProcessRdTask(chainToUpdate uint64, chainBatchIdToUpdate uint32, maxAttempts uint8) error {
	var success bool
...
func (agg *Aggregator) getL1BatchUpdateInfo(blockNumber uint32) (bool, uint64, uint32, error) {
	...
	for _, lastBatchByL1 := range substrateL2RequestsBatchLast {
		...
		chainRdBatchNonce, err := agg.ethRpc.AvsReader.ChainRdBatchNonce(context.Background(), uint64(lastBatchByL1.Key))
		if err != nil {
...
func (agg *Aggregator) verifyTaskResponseExistsOnL2(rdTaskResponse taskmanager.IFinalizerTaskManagerRdTaskResponse) error {
	...
	batchIdBytes := make([]byte, 24)
	binary.LittleEndian.PutUint64(batchIdBytes[0:], rdTaskResponse.ChainId)
	binary.LittleEndian.PutUint32(batchIdBytes[8:], rdTaskResponse.BatchId)
	...
	chainId := gsrpctypes.NewU64(rdTaskResponse.ChainId)
	rangeStart := gsrpctypes.NewU128(*rdTaskResponse.RangeStart)
	rangeEnd := gsrpctypes.NewU128(*rdTaskResponse.RangeEnd)
	err = agg.substrateClient.Client.Call(&merkleRoot, "rolldown_get_merkle_root", chainId, [2]gsrpctypes.U128{rangeStart, rangeEnd})
	...
	agg.logger.Info("Aggregator::verifyTaskResponseExistsOnL2 rdTaskResponse verified successfully", "batch_id", rdTaskResponse.BatchId, "chain", rdTaskResponse.ChainId)
