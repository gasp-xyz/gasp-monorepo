...
impl SubstrateCli for Cli {
	...
	fn load_spec(&self, id: &str) -> Result<Box<dyn sc_service::ChainSpec>, String> {
		...
		let alt1_sequencers = if self.override_alt1_sequencers.is_empty() {
			[crate::chain_spec::get_account_id_from_seed::<sp_core::ecdsa::Public>("Charleth")]
				.to_vec()
		} else {
			self.override_alt1_sequencers.iter().map(parse_accounts).collect()
		};

		let alt2_sequencers = if self.override_alt2_sequencers.is_empty() {
			[crate::chain_spec::get_account_id_from_seed::<sp_core::ecdsa::Public>("Dorothy")]
				.to_vec()
		} else {
			self.override_alt2_sequencers.iter().map(parse_accounts).collect()
		};

		Ok(match id {
			"" | "rollup-local" =>
				Box::new(chain_spec::rollup_local_config(self.randomize_chain_genesis_salt, self.chain_genesis_salt.clone(), eth_sequencers, alt1_sequencers, alt2_sequencers, EvmChain::Anvil,
				None
				)),
			"rollup-local-seq" => Box::new(chain_spec::rollup_local_config(self.randomize_chain_genesis_salt, self.chain_genesis_salt.clone(), eth_sequencers, alt1_sequencers, alt2_sequencers, EvmChain::Anvil,
				None
			)),
			"anvil" => Box::new(chain_spec::rollup_local_config(self.randomize_chain_genesis_salt, self.chain_genesis_salt.clone(), eth_sequencers, alt1_sequencers, alt2_sequencers, EvmChain::Anvil,
				None
			)),
			"reth" => Box::new(chain_spec::rollup_local_config(self.randomize_chain_genesis_salt, self.chain_genesis_salt.clone(), eth_sequencers, alt1_sequencers, alt2_sequencers, EvmChain::Reth,
				None
			)),
