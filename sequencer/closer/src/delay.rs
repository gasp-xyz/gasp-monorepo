...
async fn delay_stream_impl<'a, T: Debug + Clone>(
...
#[tracing::instrument(skip_all)]
async fn delay_stream<'a, T: Debug + Clone>(
    input: Receiver<T>,
    output: Sender<T>,
    ...
    if delay > 0 {
        delay_stream_impl(input, output, ticker, delay).await?;
    } else {
        forward_stream(input, output).await?;
    }
    Ok(())
}

#[tracing::instrument(skip_all)]
async fn forward_stream<'a, T>(mut input: Receiver<T>, output: Sender<T>) -> Result<(), ()> {
...
pub mod test {
    ...
    async fn test_forward_blocks_when_no_delay() {
        ...
        let t = tokio::spawn(async move {
            delay_stream(receiver, sender, blocks_stream.boxed(), 0)
                .await
                .unwrap();
        });
    ...
    async fn test_forward_elems_with_delay() {
        ...
        let t = tokio::spawn(async move {
            delay_stream(receiver, sender, blocks_stream.boxed(), 10)
                .await
                .unwrap();
        });
