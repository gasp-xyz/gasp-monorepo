...
mod test {
    ...
    fn dummy_withdrawal(id: u128) -> Withdrawal{
        Withdrawal{ 
            request_id: RequestId{ origin: gasp_types::Origin::L2, id: id.into() }, 
            recipient: [0u8; 20], 
    ...
    async fn test_forwards_request_created_by_frontend() {
        ...
        stash.expect_get_withdrawal_status().with(eq(non_frontend_withdrawal.withdrawal_hash())).return_once(|_| Ok(WithdrawalStatus{created_by: CreatedBy::Other}));
        stash.expect_get_withdrawal_status().with(eq(frontend_withdrawal.withdrawal_hash())).return_once(|_| Ok(WithdrawalStatus{created_by: CreatedBy::Frontend}));

        let _t = tokio::spawn(async move{
            filter_deposits_created_by_frontend(stash, input, output).await
        });
        ...
        assert_eq!(
            receiver.recv().await,
            Some(frontend_withdrawal)
        );
        assert!(
            tokio::time::timeout(Duration::from_secs_f32(0.1),receiver.recv()).await.is_err()
        );
    }
    ...
    async fn test_forwards_request_with_fee() {
        ...
        let _t = tokio::spawn(async move{
            filter_deposits_without_fee(input, output).await
        });
