...
pub struct Foo<P> {
    handle: contract_bindings::rolldown::Rolldown::RolldownInstance<BoxTransport, P>
}
...
impl FooBuilder {
    pub async fn build(self) -> Result<Foo<impl WalletProvider + Provider + Clone>, L1Error> {
        ...
        let provider = ProviderBuilder::new()
            ...
            .on_builtin(self.uri)
            .await?;
        Ok(Foo { handle: contract_bindings::rolldown::Rolldown::RolldownInstance::new( [0u8; 20].into(), provider,) })
    }
}
