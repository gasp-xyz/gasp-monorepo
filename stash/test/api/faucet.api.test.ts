...
describe('FaucetController', () => {
  ...
  it('should return 403 when captcha verification fails', async () => {
    const errorMessage =
      'Captcha verification failed. Reason: invalid-input-response'
    ...
    await supertest(app)
      ...
      .then((response) => {
        ...
        expect(responseInvalidCaptcha.exceptionName).toContain(
          'ForbiddenException'
        )
  ...
  describe('API errors', () => {
    it('should return 500 when toAddress does not start with 0x', async () => {
      ...
      await supertest(app)
        ...
        .then((response) => {
          ...
          expect(invalidAddressResponse.exceptionName).toContain(
            'ValidationError'
          )
    ...
    it('should return 403 when address is used more than 3 times', async () => {
      ...
      await supertest(app)
        ...
        .then((response) => {
          ...
          expect(responseAddressUsage.exceptionName).toContain(
            'ForbiddenException'
          )
    ...
    it('should return 500 when tokens transfer fails', async () => {
      ...
      await supertest(app)
        ...
        .then((response) => {
          ...
          expect(tokenTransferServiceFailResponse.exceptionName).toContain(
            'Error'
          )
          expect(tokenTransferServiceFailResponse.message).toContain(
            errorMessage
          )
        })
    })
    ...
    it('should return 500 when captcha service call fails', async () => {
      ...
      vi.spyOn(faucetService, 'verifyCaptcha').mockRejectedValue(
        new Error(errorMessage)
      )
