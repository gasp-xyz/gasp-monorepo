...
describe('APi tests: Coingeko', () => {
  ...
  it('GET tickers & GET pairs pool info matches', async () => {
    await supertest(app)
      ...
      .expect(200)
      .then(async (response) => {
        const pairs = (await supertest(app).get('/coingecko/pairs').expect(200))
          .body
...
describe('API Errors: Coingeko', () => {
  it('GET non existing path', async () => {
    await supertest(app).get('/coingecko/foo').expect(404)
  })
  it('POST /tickers ', async () => {
    await supertest(app).post('/coingecko/tickers').expect(404)
  })
})
...
describe.skip('System Errors: Coingeko', () => {
  it('GET /tickers  with redis down', async () => {
    ...
    await supertest(app).get('/coingecko/tickers').expect(500)
  })
})
